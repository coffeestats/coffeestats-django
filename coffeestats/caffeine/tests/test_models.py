...
class CaffeineUserManagerTest(TestCase):
    ...
    def test_create_user_empty_username(self):
        with self.assertRaisesRegex(ValueError, "User must have a username."):
            User.objects.create_user("", "")
    ...
    def test_create_user_with_password(self):
        ...
        self.assertEqual(user.token, md5("testuserpassword".encode("utf8")).hexdigest())
    ...
    def test_create_user_with_password_unicode_chars(self):
        ...
        self.assertEqual(user.token, md5("testuserpaßwört".encode("utf8")).hexdigest())
        self.assertFalse(user.is_superuser)
        self.assertFalse(user.is_staff)
        self.assertTrue(user.public)

    def test_create_superuser(self):
        user = User.objects.create_superuser("testadmin", "admin@bla.com", "s3cr3t")
        self.assertEqual(user.token, md5("testadmins3cr3t".encode("utf8")).hexdigest())
...
class UserTest(TestCase):
    ...
    def test_get_absolute_url(self):
        ...
        self.assertEqual(
            user.get_absolute_url(), reverse("public", kwargs={"username": "testuser"})
        )
    ...
    def test_export_csv(self):
        ...
        self.assertEqual(mail.outbox[0].body, "Attached is your caffeine track record.")
        self.assertEqual(mail.outbox[0].recipients()[0], "testuser@bla.com")
        self.assertEqual(len(mail.outbox[0].attachments), 2)
        self.assertRegex(mail.outbox[0].attachments[0][0], r"^coffee-.+\.csv$")
        self.assertRegex(mail.outbox[0].attachments[1][0], r"^mate-.+\.csv$")
    ...
    def test_has_usable_password_newhash(self):
        user = User.objects.create_user("testuser", "testuser@bla.com", password="test")
        self.assertTrue(user.has_usable_password())
...
class CaffeineManagerTest(TestCase):
    ...
    def _generate_caffeine_one_day(self, user):
        for hour in range(10, 18):
            coffeetime = datetime(self.now.year, self.now.month, self.now.day, hour)
            Caffeine.objects.create(
                ctype=DRINK_TYPES.coffee, date=coffeetime, user=user
            )
            if hour % 2 == 0:
                matetime = coffeetime + timedelta(minutes=30)
                Caffeine.objects.create(
                    ctype=DRINK_TYPES.mate, date=matetime, user=user
                )
    ...
    def _generate_caffeine_one_month(self, user):
        ...
        for day in range(1, monthrange(self.now.year, self.now.month)[1] + 1):
            ...
            for hour, drinktype in [
                ...
                caffeinetime = datetime(self.now.year, self.now.month, day, hour)
                Caffeine.objects.create(ctype=drinktype, date=caffeinetime, user=user)
        return labels
        ...
        for month in range(1, 13):
            for day in [
                ...
                for hour, drinktype in [
                    ...
                    caffeinetime = datetime(self.now.year, month, day, hour)
                    Caffeine.objects.create(
                        ctype=drinktype, date=caffeinetime, user=user
                    )
    ...
    def _generate_random_caffeine_multiple(self, usercount, caffeineperuser):
        for user, drinktype in [
            ...
            for day in [random.randrange(100) for item in range(caffeineperuser)]:
                caffeinetime = self.now - timedelta(days=day)
                Caffeine.objects.create(user=user, ctype=drinktype, date=caffeinetime)

    def _create_random_caffeine(self, users, number, timespan):
        drinks = dict(
            [
                (drink_type[0], {"month": 12 * [0], "hour": 24 * [0], "wday": 7 * [0]})
                for drink_type in DRINK_TYPES
            ]
        )
        maxval = {"month": 1, "hour": 1, "wday": 1}

        random.seed(0)
        for timeoffset in [
            timedelta(
                days=random.randrange(timespan.days), seconds=random.randrange(86400)
            )
    ...
    def test_total_caffeine_for_user(self):
        testuser = User.objects.create_user("testuser", "test@example.org", token="foo")
    ...
    def test_latest_caffeine_for_user(self):
        ...
        self.assertEqual(
            len([drink for drink in latest if drink.ctype == DRINK_TYPES.coffee]), 7
        )
        self.assertEqual(
            len([drink for drink in latest if drink.ctype == DRINK_TYPES.mate]), 3
        )
    ...
    def test_hourly_caffeine_for_user(self):
        testuser = User.objects.create_user("testuser", "test@example.org", token="foo")
        self._generate_caffeine_one_day(testuser)
        hourly_caffeine = Caffeine.objects.hourly_caffeine_for_user(user=testuser)
        self.assertEqual(hourly_caffeine["maxvalue"], 1)
        self.assertEqual(hourly_caffeine["labels"], [str(i) for i in range(24)])
        self.assertEqual(hourly_caffeine["coffee"], 10 * [0] + 8 * [1] + 6 * [0])
        self.assertEqual(hourly_caffeine["mate"], 10 * [0] + 4 * [1, 0] + 6 * [0])
    ...
    def test_hourly_caffeine(self):
        ...
        self.assertEqual(hourly_caffeine["labels"], [str(i) for i in range(24)])
        self.assertEqual(hourly_caffeine["coffee"], 10 * [0] + 8 * [5] + 6 * [0])
        self.assertEqual(hourly_caffeine["mate"], 10 * [0] + 4 * [5, 0] + 6 * [0])
    ...
    def test_monthly_caffeine_for_user(self):
        ...
        monthly_caffeine = Caffeine.objects.monthly_caffeine_for_user(user=testuser)
        self.assertEqual(monthly_caffeine["maxvalue"], 20)
        self.assertEqual(
            monthly_caffeine["labels"], [str(month) for month in range(1, 13)]
        )
    ...
    def test_hourly_caffeine_for_user_overall(self):
        user = User.objects.create_user("testuser", "test@example.org", token="foo")
        drinks, maxval = self._create_random_caffeine(
            users=[user], number=100, timespan=timedelta(days=365)
        )
        hourly_caffeine = Caffeine.objects.hourly_caffeine_for_user_overall(user=user)
        self.assertEqual(hourly_caffeine["maxvalue"], maxval["hour"])
        self.assertEqual(hourly_caffeine["labels"], [str(hour) for hour in range(24)])
        self.assertEqual(hourly_caffeine["coffee"], drinks[DRINK_TYPES.coffee]["hour"])
        self.assertEqual(hourly_caffeine["mate"], drinks[DRINK_TYPES.mate]["hour"])
    ...
    def test_weekdaily_caffeine_for_user_overall(self):
        ...
        weekdaily_caffeine = Caffeine.objects.weekdaily_caffeine_for_user_overall(
            user=user
        )
        self.assertEqual(weekdaily_caffeine["maxvalue"], maxval["wday"])
        self.assertEqual(weekdaily_caffeine["labels"], WEEKDAY_LABELS)
        self.assertEqual(
            weekdaily_caffeine["coffee"], drinks[DRINK_TYPES.coffee]["wday"]
        )
        self.assertEqual(weekdaily_caffeine["mate"], drinks[DRINK_TYPES.mate]["wday"])
    ...
    def test_find_recent_caffeine_no_caffeine(self):
        user = User.objects.create_user("testuser", "test@example.org", token="foo")
        Caffeine.objects.create(
            user=user, ctype=DRINK_TYPES.coffee, date=self.now - timedelta(days=2)
        )
        Caffeine.objects.create(user=user, ctype=DRINK_TYPES.mate, date=self.now)
        self.assertFalse(
    ...
    def test_find_recent_caffeine_caffeine(self):
        ...
        latest = Caffeine.objects.create(
            user=user, ctype=DRINK_TYPES.coffee, date=self.now - timedelta(seconds=60)
        )
        self.assertEqual(
            Caffeine.objects.find_recent_caffeine(
                user=user, date=self.now, ctype=DRINK_TYPES.coffee
    ...
    def test_latest_caffeine_activity(self):
        ...
        ref = [
            drink.id
            for drink in sorted(drinks, key=lambda x: x.date, reverse=True)[:-10]
        ]
        self.assertEqual(
            [drink.id for drink in Caffeine.objects.latest_caffeine_activity()], ref
        )
    ...
    def test_top_consumers_total(self):
        ...
        for user in users:
            for _ in range(coffeecount):
                Caffeine.objects.create(
                    ...
                    date=self.now
                    - timedelta(
                        days=random.randrange(100), seconds=random.randrange(86400)
                    ),
                )
            coffeecount -= 1
        ...
        self.assertEqual([item["user"] for item in toptotal], users[:10])
        self.assertEqual(
            [item["caffeine_count"] for item in toptotal], list(range(20, 10, -1))
        )
        toptotal = Caffeine.objects.top_consumers_total(DRINK_TYPES.mate)
        self.assertEqual(
            [item["user"] for item in toptotal], list(reversed(users[-10:]))
        )
        self.assertEqual(
            [item["caffeine_count"] for item in toptotal], list(range(20, 10, -1))
        )
    ...
    def test_top_consumers_average(self):
        ...
        self.assertEqual([item["user"] for item in topavg], users[:10])
        ...
        self.assertEqual([item["user"] for item in topavg], list(reversed(users[-10:])))
        averages = [item["average"] for item in topavg]
        self.assertTrue(averages, sorted(averages, reverse=True))
    ...
    def test_top_consumers_recent(self):
        ...
        self.assertEqual(
            top_averages, [1.8, 1.7, 1.2, 1.2, 1.0, 0.9, 0.7, 0.6, 0.6, 0.4]
        )
        self.assertEqual(top_totals, [18, 17, 12, 12, 10, 9, 7, 6, 6, 4])
        ...
        self.assertEqual(
            top_averages, [1.8, 1.4, 1.1, 0.9, 0.8, 0.6, 0.5, 0.4, 0.3, 0.2]
        )
        self.assertEqual(top_totals, [18, 14, 11, 9, 8, 6, 5, 4, 3, 2])

    def test_get_csv_data(self):
        user = User.objects.create_user(username="test", email="test@example.org")
        td = timedelta(days=1)
        coffees = sorted(
            [
                Caffeine.objects.create(
...
class CaffeineTest(TransactionTestCase):
    ...
    def test___str___without_timezone(self):
        user = User.objects.create_user("testuser", "test@example.org")
        caff = Caffeine.objects.create(
            ctype=DRINK_TYPES.coffee, date=timezone.now(), user=user
        )
        self.assertRegex(
            str(caff),
            r"^%s at \d{4}-\d{2}-\d{2} [^ ]+$" % (DRINK_TYPES[DRINK_TYPES.coffee],),
        )

    def test___str___with_timezone(self):
        user = User.objects.create_user("testuser", "test@example.org")
        caff = Caffeine.objects.create(
            ctype=DRINK_TYPES.mate, date=timezone.now(), user=user, timezone="GMT"
        )
        self.assertRegex(
            str(caff),
            r"^%s at \d{4}-\d{2}-\d{2} [^ ]+ GMT$" % (DRINK_TYPES[DRINK_TYPES.mate],),
        )
    ...
    def test_format_type(self):
        ...
        self.assertEqual(caff.format_type(), DRINK_TYPES[DRINK_TYPES.coffee])
    ...
    def test_clean(self):
        ...
        with self.assertRaisesRegex(
            ValidationError, r"Your last \w+ was less than \d+ minutes ago at"
        ):
            second_caff.clean()
...
class ActionManagerTest(TestCase):
    def test_create_action(self):
        ...
        for actiontype in list(ACTION_TYPES):
            action = Action.objects.create_action(user, actiontype[0], "test", 10)
            self.assertEqual(len(action.code), 32)
            self.assertLessEqual(action.validuntil, timezone.now() + timedelta(10))
