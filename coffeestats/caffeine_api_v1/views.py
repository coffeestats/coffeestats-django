...
def api_token_required(func):
    ...
    def inner(request, *args, **kwargs):
        ...
        if "error" in messages:
            ...
            return HttpResponseForbidden(json.dumps(messages), "text/json")
        user = None
        try:
            user = User.objects.get(
                username=request.POST.get("u"), token=request.POST.get("t")
            )
        except:
            messages.setdefault("error", []).append(API_ERROR_INVALID_CREDENTIALS)
            return HttpResponseBadRequest(json.dumps(messages), "text/json")
        if not user.timezone:
            messages.setdefault("warning", []).append(API_WARNING_TIMEZONE_NOT_SET)
...
def add_drink(request, userinfo, messages, *args, **kwargs):
    ...
    time = None
    if not ctype:
        messages.setdefault("error", []).append(API_ERROR_MISSING_PARAM_BEVERAGE)
    elif not hasattr(DRINK_TYPES, ctype):
        messages.setdefault("error", []).append(API_ERROR_INVALID_BEVERAGE)
    if not drinktime:
        messages.setdefault("error", []).append(API_ERROR_MISSING_PARAM_TIME)
    else:
    ...
    if not form.is_valid():
        ...
        return HttpResponseBadRequest(json.dumps(messages), "text/json")
    drink = form.save()
    messages["success"] = _("Your %(drink)s has been registered!") % {"drink": drink}
    return messages
