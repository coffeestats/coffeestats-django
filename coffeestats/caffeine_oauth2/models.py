...
class CoffeestatsApplication(AbstractApplication):
    ...
    logo = models.ImageField(verbose_name=_("application logo"), upload_to="appimages")
    agree = models.BooleanField(
        verbose_name=_("accept API usage agreement"),
        help_text=mark_safe(
            _(
                'You have to agree to the <a href="%s">API usage agreement</a>'
                " to use our APIs."
            )
            % settings.API_USAGE_AGREEMENT
        ),
    )
    ...
    approved = models.BooleanField(
        verbose_name=_("approved by coffeestats team"), default=False
    )
    ...
    class Meta(AbstractApplication.Meta):
        ...
        permissions = (("can_approve", _("Can approve applications")),)
