...
class RegisterApplicationTest(BaseCoffeeStatsPageTestMixin, SeleniumTest):
    ...
    def goto_application_registration(self):
        ...
        header = self.selenium.find_element(by=By.ID, value="header")
        nav = header.find_element(by=By.TAG_NAME, value="nav")
        settings_menu = nav.find_element(by=By.CLASS_NAME, value="settings")
        action_chain = ActionChains(self.selenium)
        action_chain.move_to_element(settings_menu).perform()
        settings_link = nav.find_element(by=By.LINK_TEXT, value="Settings")
        settings_link.click()
        ...
        reglink = self.selenium.find_element(
            by=By.LINK_TEXT, value="Register a new application"
        )
        reglink.click()

        # user sees a form
        regform = self.selenium.find_element(by=By.TAG_NAME, value="form")
        self.assertTrue(
    ...
    def test_register_and_approve_oauth2_client(self):
        ...
        self.assertEqual(client_type_field.get_attribute("id"), "id_client_type")
        ...
        self.assertEqual(redirect_uris_field.get_attribute("id"), "id_redirect_uris")
        redirect_uris_field.send_keys(
            "https://coffeeapp.example.org/auth/"
            + Keys.ENTER
            + "org.coffeestats.cli://auth/"
            + Keys.TAB
        )
        ...
        application_id = re.search(
            r"(?<=/oauth2/applications/)\d+", self.selenium.current_url
        ).group(0)
        ...
        approval_url = reverse("oauth2_provider:approve", kwargs={"pk": application_id})
        ...
        self.assertRegexpMatches(self.selenium.current_url, r"/login/\?next=")
        self.assertTrue(self.selenium.current_url.endswith(urlquote(approval_url)))
        ...
        approve_button = self.selenium.find_element(by=By.TAG_NAME, value="button")
        ...
        self.assertRegexpMatches(self.selenium.current_url, r"/login/\?next=")
        self.assertTrue(
            self.selenium.current_url.endswith(urlquote(application_detail_url))
        )
        ...
        self.assertTrue(self.selenium.current_url.endswith(application_detail_url))
        ...
        self.assertTrue(self.selenium.current_url.endswith(application_list_url))

        # check that the requested application is listed
        self.assertIn(
            "/oauth2/applications/{}/".format(application_id), self.selenium.page_source
        )
    ...
    def test_register_and_reject_oauth2_client(self):
        ...
        self.assertIn(
            "You have to agree to our API usage agreement", self.selenium.page_source
        )
        agree_field = self.selenium.find_element(by=By.ID, value="id_agree")
        action_chain = ActionChains(self.selenium)
        action_chain.click(agree_field).perform()

        submit_button = self.selenium.find_element(by=By.TAG_NAME, value="button")
        self.assertEqual(submit_button.get_attribute("type"), "submit")
        action_chain = ActionChains(self.selenium)
        ...
        reject_button = self.selenium.find_element(by=By.LINK_TEXT, value="Reject")
        self.assertEqual(reject_button.tag_name, "a")
        self.assertRegexpMatches(
            reject_button.get_attribute("href"), r"/oauth2/applications/\d+/reject/"
        )
        reject_button.click()

        # find the rejection form
        reject_form = self.selenium.find_element(by=By.TAG_NAME, value="form")
        reasoning_field = reject_form.find_element(by=By.ID, value="id_reasoning")
        reasoning_field.send_keys(
            "Your application does not fulfill our quality criteria." + Keys.TAB
        )

        reject_button = self.selenium.find_element(by=By.TAG_NAME, value="button")
        ...
        self.assertRegexpMatches(self.selenium.current_url, r"/login/\?next=")
        self.assertTrue(
            self.selenium.current_url.endswith(urlquote(application_list_url))
        )
