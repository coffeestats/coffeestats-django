...
EMAIL_CHANGE_SUCCESS_MESSAGE = _("Your email address has been changed successfully.")
        ...
        context_data.update(
            {
                ...
                "topmate": Caffeine.objects.top_consumers_total(DRINK_TYPES.mate, 10),
                ...
                "longestjoined": User.objects.longest_joined(count=5, days=365),
            }
        )
        return context_data
...
class ExportActivityView(LoginRequiredMixin, RedirectView):
    ...
    def get_redirect_url(self, *args, **kwargs):
        ...
        messages.add_message(self.request, messages.INFO, EXPORT_SUCCESS_MESSAGE)
        return super(ExportActivityView, self).get_redirect_url(*args, **kwargs)
...
class DeleteAccountView(LoginRequiredMixin, DeleteView):
    ...
    def get_success_url(self):
        logout(self.request)
        messages.add_message(self.request, messages.INFO, DELETE_ACCOUNT_MESSAGE)
...
class PublicProfileView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if "username" in self.kwargs:
            self.profileuser = get_object_or_404(User, username=self.kwargs["username"])
        else:
            self.profileuser = self.request.user

        total = Caffeine.objects.total_caffeine_for_user(self.profileuser)
        todaydata = Caffeine.objects.hourly_caffeine_for_user(self.profileuser)
        monthdata = Caffeine.objects.daily_caffeine_for_user(self.profileuser)
        yeardata = Caffeine.objects.monthly_caffeine_for_user(self.profileuser)
        byhourdata = Caffeine.objects.hourly_caffeine_for_user_overall(self.profileuser)
...
class ProfileView(PublicProfileView):
    ...
    def get(self, request, *args, **kwargs):
        if "u" in request.GET:
            return HttpResponseRedirect(
                reverse("public", kwargs={"username": request.GET["u"]})
            )
    ...
    def get_context_data(self, **kwargs):
        ...
        entries = Caffeine.objects.latest_caffeine_for_user(self.profileuser)
        context.update(
            {
                "entries": entries,
            }
...
class CaffeineActivationView(ActivationView):
    def get_success_url(self, user=None):
        messages.add_message(self.request, messages.SUCCESS, ACTIVATION_SUCCESS_MESSAGE)
...
class CaffeineRegistrationView(RegistrationView):
    ...
    def get_success_url(self, user=None):
        ...
        messages.add_message(self.request, messages.INFO, REGISTRATION_MAILINFO_MESSAGE)
        return reverse_lazy("home")
    ...
    def send_email_change_mail(self, form):
        ctx_dict = {
            ...
            "action_link": self.request.build_absolute_uri(
                reverse("confirm_action", kwargs={"code": form.email_action.code})
            ),
        ...
        body = render_to_string("django_registration/email_change_email.txt", ctx_dict)
        form.instance.email_user(subject, body, settings.DEFAULT_FROM_EMAIL)
        messages.add_message(self.request, messages.INFO, SETTINGS_EMAIL_CHANGE_MESSAGE)

    def form_valid(self, form):
        messages.add_message(self.request, messages.SUCCESS, SETTINGS_SUCCESS_MESSAGE)
        if form.email_action:
            self.send_email_change_mail(form)
        form.save()
        if form.password_set:
            messages.add_message(
                self.request, messages.SUCCESS, SETTINGS_PASSWORD_CHANGE_SUCCESS
            )
...
class ConfirmActionView(SingleObjectMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if action.atype == ACTION_TYPES.change_email:
            user.email = data
            messages.add_message(
                request, messages.SUCCESS, EMAIL_CHANGE_SUCCESS_MESSAGE
            )
            user.save()
            action.delete()
...
class OnTheRunOldView(RedirectView):
    ...
    def get_redirect_url(self, *args, **kwargs):
        user = get_object_or_404(
            User, username=self.request.GET.get("u"), token=self.request.GET.get("t")
        )
        return reverse(
            "ontherun", kwargs={"username": user.username, "token": user.token}
        )
...
class BaseSubmitCaffeineView(BaseFormView):
    ...
    def form_valid(self, form):
        ...
        messages.add_message(
            self.request,
            messages.SUCCESS,
            SUBMIT_CAFFEINE_SUCCESS_MESSAGE % {"caffeine": caffeine},
    ...
    def form_invalid(self, form):
        for field in form.errors:
            for error in form.errors[field]:
                messages.add_message(
                    self.request, messages.ERROR, error, extra_tags="registerdrink"
                )
...
class SubmitCaffeineOnTheRunView(BaseSubmitCaffeineView):
    ...
    def get_success_url(self):
        return reverse_lazy(
            "ontherun",
            kwargs={"username": self.kwargs["username"], "token": self.kwargs["token"]},
        )
