...
class SettingsFormTest(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            username="testuser", email="test@bla.com", password="test1234"
        )
    ...
    def test_clean_password2_valid(self):
        self.assertTrue(self.user.check_password("test1234"))
        form = SettingsForm(
            data={"password1": "test2345", "password2": "test2345"}, instance=self.user
        )
        self.assertTrue(form.is_valid())
    ...
    def test_clean_password2_invalid(self):
        self.assertTrue(self.user.check_password("test1234"))
        form = SettingsForm(
            data={"password1": "test2345", "password2": "wrongone"}, instance=self.user
        )
        self.assertFalse(form.is_valid())
    ...
    def test_clean_password2_one_empty(self):
        form = SettingsForm(
            data={"password1": "test2345", "password2": ""}, instance=self.user
        )
        self.assertFalse(form.is_valid())
        self.assertEqual(form.errors["password2"], [PASSWORD_MISMATCH_ERROR])
        self.assertTrue(self.user.check_password("test1234"))
    ...
    def test_clean_email_no_change(self):
        form = SettingsForm(data={"email": "test@bla.com"}, instance=self.user)
        self.assertTrue(form.is_valid())

    def test_clean_email_change_user(self):
        form = SettingsForm(data={"email": "test@blub.com"}, instance=self.user)
        self.assertTrue(form.is_valid())
        ...
        self.assertEqual(Action.objects.all()[0].atype, ACTION_TYPES.change_email)
...
class SubmitCaffeineFormTest(TestCase):
    ...
    def test_clean_no_previous(self):
        ...
        form = SubmitCaffeineForm(
            self.user, DRINK_TYPES.coffee, data={"date": now.date(), "time": now.time()}
        )
        self.assertTrue(form.is_valid(), str(form.errors))
    ...
    def test_clean_recent_previous(self):
        Caffeine.objects.create(
            ...
            ctype=DRINK_TYPES.coffee,
            date=timezone.now()
            - timedelta(minutes=settings.MINIMUM_DRINK_DISTANCE - 1),
        )
        now = timezone.now()
        form = SubmitCaffeineForm(
            self.user, DRINK_TYPES.coffee, data={"date": now.date(), "time": now.time()}
    ...
    def test_submit_older(self):
        Caffeine.objects.create(
            user=self.user, ctype=DRINK_TYPES.coffee, date=timezone.now()
        )
        before = timezone.now() - timedelta(minutes=settings.MINIMUM_DRINK_DISTANCE + 1)
        form = SubmitCaffeineForm(
            self.user,
            DRINK_TYPES.coffee,
    ...
    def test_clean_empty_date_and_time(self):
        caffeine = SubmitCaffeineForm(
            self.user, DRINK_TYPES.coffee, data={"date": "", "time": ""}
        ).save()
        self.assertLessEqual(caffeine.date, timezone.now())
        self.assertEqual(caffeine.ctype, DRINK_TYPES.coffee)
        self.assertEqual(caffeine.user, self.user)
